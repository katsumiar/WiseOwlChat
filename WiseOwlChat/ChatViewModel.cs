    ...
    public partial class ChatViewModel : INotifyPropertyChanged
    {
        ...
        private async Task preQuery(Action update, Action<string?>? callback, string message, Directions? direction)
        {
            ...
            if (error != null)
            {
                ...
                AppendMessageToOpenAIChat(ConversationEntry.ROLE_SYSTEM, $"The user tried to disclose {error}, but it was restricted by the WiseOwlChat system. The disclosure of {error} is prohibited. Please guide the user to avoid the prohibited action (speak in Japanese).");
